...
server <- function(input, output) {
  ...
  observeEvent(input$RunPA, {  #
    ...
    withProgress(message = 'Performing probabilistic analysis', value = 0, {
      ...
      # ## Run parallelized PA based on OS
      if(os == "macosx"){
        ...
        df_ce <- foreach::foreach(g = 1:input$n_iter, .combine = rbind) %dopar% {
          ...
          # Here we add an extra arrow -> Store in the base environment and stored with this values
