...
plot_survival <- function(df_survival, cycle_zoom = FALS){
  ...
  # cycle_zoom: zoom in to the first x cycles, default is FALSE. If is is a number zoom in up to this cycle
    ...
    if(cycle_zoom == FALSE){
      ...
      plot <- ggplot(data = df_survival, aes(x = cycle, y = OS, group = strategy)) + 
        geom_line(aes(linetype = strategy, color = strategy)) +
        ylab ("Overall survival") + 
        ggtitle(paste("Overall survival", remove_date_from_name(df_survival$treatment[1]), seb = ""))
    }
    ...
    if (cycle_zoom > 0){
      ...
      plot <- ggplot(data = df_survival, aes(x = cycle, y = OS, group = strategy)) +  
        ...
        ylab ("Overall survival") + 
        ggtitle(paste("Overall survival - first 5 cycles", remove_date_from_name(df_survival$treatment[1]), seb = ""))
    }
...
plot_life_expectancy <- function(df_le_plot){
  ...
  plot <- ggplot(data = df_le_plot, aes(x = strategy, y = LE, group = strategy, fill = strategy)) + 
    geom_col() +
    ggtitle(paste("Life expectancy", remove_date_from_name(n), sep = " ")) 
